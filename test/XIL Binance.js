...
describe("Token", async () => {
  ...
  beforeEach(async () => {
    ...
    [owner, addr1, addr2, addr3, treasuryAddr, idoAddr, pairAddr, ...addrs] = await ethers.getSigners();
...
//         await expect(token.transferWhitelister(treasuryAddr.address)).to.emit(token, 'WhitelisterTransferred')
//           .withArgs(owner.address, treasuryAddr.address);
//       });
...
//         await expect(
//           token.transferWhitelister(AddressZero)
//         ).to.be.revertedWith("New whitelister is the zero address");
//       });
//     });

//     describe('renounce ownership', () => {
//       it('renounce owner', async () => {
//         await expect(token.renounceWhitelister()).to.emit(token, 'WhitelisterTransferred')
//           .withArgs(owner.address, AddressZero);
//       });

//       it('renounce non owner', async () => {
//         await expect(
//           token.connect(addr1).renounceWhitelister()
//         ).to.be.revertedWith("Caller is not the whitelister");
...
//   });
  ...
  describe("Transactions - Whitelist POPULATED", () => {
    ...
    it("Should reject bad whiteList data", async () => {
      ...
      await expect(token.connect(idoAddr).createLGEWhitelist(addr1.address, durations, amountsMax)).to.be.revertedWith("revert Invalid whitelist(s)");
    });
    ...
    it("transferring tokens to the ido address begins the LGE", async () => {
      ...
      const addresses = [pairAddr.address, idoAddr.address, owner.address, addr1.address, addr2.address];
